...
fn build_stepping_hint(mut commands: Commands, asset_server: Res<AssetServer>) {
    ...
    // } else {
    //     "Bevy was compiled without stepping support. Run with `--features=bevy_debug_stepping` to \
    //      enable stepping."
    // };

    let hint_text = "Press F6 to toggle stepping mode (S: step system, F7: step frame)";
    info!("{}", hint_text);
